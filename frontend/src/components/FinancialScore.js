...
export default function FinancialScore({ score }) {
  const getColor = (s) => {
    ...
    if (s >= 60) return { text: 'text-blue-600', ring: '#3b82f6', label: 'Good', bg: 'bg-blue-50' };
    if (s >= 40) return { text: 'text-yellow-500', ring: '#f59e0b', label: 'Fair', bg: 'bg-yellow-50' };
  ...
  return (
    <Link href="/dashboard/score">
      <div className="group bg-white rounded-2xl border border-gray-200 shadow-sm p-6 hover:shadow-md hover:border-emerald-200 transition-all duration-300 cursor-pointer">
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="mt-2 text-xs text-gray-400 group-hover:text-emerald-600 transition-colors">
            View breakdown →
          </span>
        </div>
      </div>
